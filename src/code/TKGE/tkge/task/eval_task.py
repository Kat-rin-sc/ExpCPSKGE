...
class EvaluateTask(Task):
    ...
    def main(self):
        ...
        df = pd.DataFrame(columns = ["Side", "Type", "Metric", "Value"])

        eval_datasets = ["inferred", "explanation", "events", "all"]

        for e in eval_datasets:
            self.config.log(f"Starting {e} Evaluation")
            self.test_loader = torch.utils.data.DataLoader(
            SplitDataset(self.dataset.get(e), self.datatype + ['timestamp_id']),
            shuffle=False,
            batch_size=self.test_bs
        )

            # evaluating optimistic ordering
            for type in ["optimistic", "pessimistic"]:
                self.config.log(f"Starting {type} Evaluation")
                self.config.set("eval.preference", type)
                metrics = self.eval()
                ...
                for side in ["head", "tail", "avg"]:
                    for m,v in metrics[side].items():
                        df.loc[len(df)] = [side, type, m,v]
            
            df.to_csv(self.config.get("task.folder")+"/TTransE_default_" + e +".csv")
