...
class PredictTask(Task):
    ...
    def parse_arguments(parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        description = """Predict from a model"""
        subparser = parser.add_parser("pred", description=description, help="predict from a model.")
        ...
        subparser.add_argument(
            ...
            "--head",
            type=str,
            help="specify the head of the prediction",
            dest='head'
        )
        ...
        subparser.add_argument(
            ...
            "--rel",
            type=str,
            help="specify the relation of the prediction",
            dest='relation'
        )
    ...
    def _prepare(self):
        ...
        self.evaluation = Prediction(config=self.config, dataset=self.dataset)
    ...
    def main(self):
        self.config.log("BEGIN Predicting")
#------------------------------------------------------------------------------------------------------------------------------------------------
