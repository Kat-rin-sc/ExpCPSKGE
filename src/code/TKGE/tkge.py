...
from tkge.task.predict_task import PredictTask
...
parser_pred = PredictTask.parse_arguments(subparsers)
...
config_path = args.config if args.task not in  ['resume', 'eval', 'pred'] else os.path.join(args.experiment, 'config.yaml')
