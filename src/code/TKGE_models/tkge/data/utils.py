...
def get_all_hours_of_days(day_start: int, day_end: int, year: int = 1973, month: int= 12, format: str = "YYYY-MM-DDTHH:mm:ss") -> List[str]:
    """
    get all days of the year in string format
    """

    start_date = '%s-%s-%sT00:00:40' % (year, month, day_start)
    a = 0
    all_date_list = []
    hours_sum = (day_end-day_start+1)*24
    while a < hours_sum:
        b = arrow.get(start_date).shift(hours=a).format(format)
        a += 1
        all_date_list.append(b)
