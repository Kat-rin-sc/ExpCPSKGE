...
class TTransEModel(BaseModel):
    ...
    def predict_candidates(self, queries: torch.Tensor):
        self.config.assert_true(torch.isnan(queries).sum(1).byte().all(), "Either head or tail should be absent.")
        ...
        scores, _ = self.forward(candidates)
        tail_candidates = candidates.numpy()[:,2]
        scores = scores.view(bs, -1)
        #print(tail_candidates.shape)
        #print(scores.shape)

        return tail_candidates, scores.numpy()
    
    def predict_triples(self, queries: torch.Tensor):
        self.config.assert_true(torch.isnan(queries).sum(1).byte().all(), "Either head or tail should be absent.")
